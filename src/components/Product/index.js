...
export default function Product(props) {
  const { id, name, price, score, image, favorite, cart, quantity } = props;

  const dispatch = useDispatch();

  const haveInCart = useSelector((state) =>
    state.cart.some((productInCart) => productInCart.id === id)
  );
      ...
      <div className={styles["product-info"]}>
        ...
        <div className={styles["product-actions"]}>
          <span className={styles["product-action"]}>
            ...
            {cart ? (
              <FaTrashAlt size={20} color={"FF0000"} onClick={resolveCart} />
            ) : null}
          </span>
          <span className={styles["product-action"]}>
            {cart ? (
